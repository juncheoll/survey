...
To control benchmark token lengths, use `--test-input-tokens` and `--max-new-tokens`:
```
python run_exp.py --top_p 0.9 --temperature 0.6 --gen_type SpecExecBase --max_budget 128 --test-input-tokens 1024 --max-new-tokens 256 --n_tests=1 --offload --exp_name="SX_sample_offload_1024_256"
```

Add `--ignore-eos` to keep generating until `--max-new-tokens` instead of stopping at EOS:
```
python run_exp.py --top_p 0.9 --temperature 0.6 --gen_type SpecExecBase --max_budget 128 --test-input-tokens 1024 --max-new-tokens 256 --ignore-eos --n_tests=1 --offload --exp_name="SX_sample_offload_1024_256"
```
