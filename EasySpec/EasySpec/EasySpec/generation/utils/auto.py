...
class MyGenerationMixin(GenerationMixin):
    ...
    def _sample(
        ...
        **model_kwargs,
        ...
        if do_sample is True and not isinstance(logits_processor, LogitsProcessorList):
            raise ValueError(
                "`do_sample` is set to `True`, `logits_processor` must be a `LogitsProcessorList` instance (it is "
                f"{logits_processor})."
            )
        ...
        # set forward for batch acc
        if self.config.is_batch_accelerate == True:
            for decoder_layer in self.model.layers:
                decoder_layer.forward = decoder_layer.prefilling_forward
                # decoder_layer.forward = decoder_layer.forward_basic
        ...
        while self._has_unfinished_sequences(
            ...
            if do_sample:
                ...
                # print(next_token_scores.softmax(dim=-1).max(dim=-1).values.item())
