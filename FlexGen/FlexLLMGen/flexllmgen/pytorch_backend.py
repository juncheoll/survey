...
class TorchDevice:
    ...
    def llama_input_embed(self, inputs, attention_mask, w_token, donate):
        """Llama input embedding (no position embeddings, RoPE is applied in attention)."""
        # decompress weights
        if w_token.device.device_type == DeviceType.COMPRESSED:
            w_token = w_token.device.decompress(w_token)
    ...
    def llama_output_embed(self, inputs, w_ln, w_token, donate,
        ...
        """Llama output embedding with RMSNorm."""
        # decompress weights
        if w_token.device.device_type == DeviceType.COMPRESSED:
            w_token = w_token.device.decompress(w_token)
        ...
        # RMSNorm
        rms = torch.sqrt(torch.mean(inputs.data ** 2, dim=-1, keepdim=True) + eps)
        hidden = inputs.data / rms * w_ln.data
        if donate[0]: inputs.delete()
    ...
    def _apply_rope(self, q, k, seq_len, rope_theta=10000.0):
        """Apply Rotary Position Embedding (RoPE) to query and key tensors.
        
        RoPE applies a rotation to each position based on the position index.
        """
        ...
        inv_freq = 1.0 / (rope_theta ** (torch.arange(0, head_dim, 2, dtype=torch.float32, device=self.dev) / head_dim))
        
        # Generate position indices: shape (seq_len,)
        positions = torch.arange(seq_len, dtype=torch.float32, device=self.dev)
        
        # Compute frequency embeddings: (seq_len, head_dim // 2)
        freqs = torch.einsum("i,j->ij", positions, inv_freq)
        
        # Create emb: (seq_len, head_dim) by interleaving cos and sin
        # e.g., for head_dim=4: [cos(pos*freq0), cos(pos*freq0), cos(pos*freq1), cos(pos*freq1), sin(pos*freq0), sin(pos*freq0), sin(pos*freq1), sin(pos*freq1)]
        emb = torch.cat([freqs, freqs], dim=-1)
        
        cos_cached = torch.cos(emb).to(dtype=q.dtype)  # (seq_len, head_dim)
        sin_cached = torch.sin(emb).to(dtype=q.dtype)  # (seq_len, head_dim)
        
        # Apply rotation: q_rot = q * cos + rotate_half(q) * sin
        q_rot = (q * cos_cached.unsqueeze(0)) + (self._rotate_half(q) * sin_cached.unsqueeze(0))
        k_rot = (k * cos_cached.unsqueeze(0)) + (self._rotate_half(k) * sin_cached.unsqueeze(0))
    ...
    def _rms_norm(self, hidden_states, weight, eps=1e-6):
    ...
    def llama_mha(self, inputs, attention_mask, w_q, w_k, w_v,
        ...
        hidden = self._rms_norm(inputs.data, w_ln.data, eps=eps)
    ...
    def llama_mha_gen(self, inputs, attention_mask, w_q, w_k, w_v,
        ...
        q_rot = q_rot[:, -tgt_s:, :]
        k_new = k_rot[:, -tgt_s:, :]
        k_new_cache = k_new.permute(1, 0, 2)
        v_new_cache = v_new.permute(1, 0, 2)

        if isinstance(k_cache, TorchTensor):
            if attn_sparsity >= 1.0:  # Dense attention
                if compress_cache:
                    k = k_cache.device.decompress(k_cache)[:past_s]
                    v = v_cache.device.decompress(v_cache)[:past_s]
                else:
                    k = k_cache.data[:past_s]
                    v = v_cache.data[:past_s]
                
                # Concatenate with new cache
                ...
                # Causal mask
                idx = torch.arange(src_s, device=self.dev)
                causal_mask = (idx[-tgt_s:] <= idx).view(1, tgt_s, src_s)
                mask = attention_mask.data.view(b, 1, src_s) & causal_mask
                
                # Apply mask
                attn_weights = attn_weights.view(b, n_head, tgt_s, src_s)
                attn_weights = torch.where(mask.view(b, 1, tgt_s, src_s), attn_weights, -1e4)
                attn_weights = attn_weights.view(b * n_head, tgt_s, src_s)
                attn_weights = F.softmax(attn_weights, dim=2)

                value = torch.bmm(attn_weights, v_t)
                value = value.view(b, n_head, tgt_s, head_dim)
                ...
                # Store new cache
                k_new = TorchTensor.create_from_torch(k_new_cache, self) if not compress_cache else self.compressed_device.compress(k_new_cache, comp_config)
                v_new = TorchTensor.create_from_torch(v_new_cache, self) if not compress_cache else self.compressed_device.compress(v_new_cache, comp_config)

                return TorchTensor.create_from_torch(value, self), k_new, v_new

        # Fallback to simple cache update
        k_new = TorchTensor.create_from_torch(k_new_cache, self) if not compress_cache else self.compressed_device.compress(k_new_cache, comp_config)
        v_new = TorchTensor.create_from_torch(v_new_cache, self) if not compress_cache else self.compressed_device.compress(v_new_cache, comp_config)
        return TorchTensor.create_from_torch(torch.zeros_like(inputs.data), self), k_new, v_new

    def llama_mlp(self, inputs, w_gate, w_up, w_down, w_ln, donate, eps=1e-5):
        """Llama MLP with RMSNorm and SiLU activation.
        
        Llama MLPUse gate_proj (with SiLU), up_proj, and down_proj:
        ...
        # RMSNorm
        out = self._rms_norm(inputs.data, w_ln.data, eps=eps)
        ...
        # Down projection
        out = F.linear(hidden, w_down.data)
