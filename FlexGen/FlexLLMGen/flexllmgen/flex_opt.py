...
from flexllmgen.opt_config import (OptConfig, get_opt_config, LlamaConfig,
    get_llama_config, get_model_config, download_opt_weights,
    download_llama_weights)
...
class LlamaInputEmbed:
    ...
    def forward(self, hidden, cache_read_buf, weight_read_buf, attention_mask,
        ...
        if k == self.policy.num_gpu_batches - 1:
            ...
            (w_token, _) = weight_read_buf.val[0]

        h = self.compute.llama_input_embed(h, mask, w_token, donate)
        hidden.val = h


class LlamaOutputEmbed:
    def __init__(self, config, env, policy):
        self.config = config
        self.env = env
        self.policy = policy
    ...
    def init_weight(self, weight_home, path):
        v, h, dtype = (self.config.vocab_size, self.config.input_dim,
            self.config.dtype)
        path = os.path.join(path, "")
        lm_head_path = path + "lm_head.weight"
        if not os.path.exists(lm_head_path):
            lm_head_path = path + "model.embed_tokens.weight"
        weight_specs = [
            # w_ln (norm weight only, no bias)
            ((h,), dtype, path + "model.norm.weight"),
            # w_token (lm_head.weight)
            ((v, h), dtype, lm_head_path),
        ]
        weights = init_weight_list(weight_specs, self.policy, self.env)
    ...
    def forward(self, hidden, cache_read_buf, weight_read_buf, attention_mask,
        ...
        h = self.compute.llama_output_embed(h, w_ln, w_token, donate,
            self.task.do_sample, self.task.temperature, self.config.layer_norm_eps)
        hidden.val = h
...
class LlamaSelfAttention:
    ...
    def load_weight(self, weight_home, weight_read_buf, k):
        ...
        if k == 0:
            ...
            dst2 = self.compute
            weight_read_buf.store((
                w_q.smart_copy(dst1), w_k.smart_copy(dst1), w_v.smart_copy(dst1),
    ...
    def load_cache(self, cache_home, cache_read_buf, i):
        ...
        k_cache, v_cache = cache_home.val
        dst = self.attention_compute
        indices = (slice(0, self.task.prompt_len + i),
                   slice(0, k_cache.shape[1]))
        cache_read_buf.store((
            k_cache.smart_copy(dst, indices),
            v_cache.smart_copy(dst, indices),
        ))
    ...
    def forward(self, hidden, cache_read_buf, weight_read_buf, attention_mask,
        ...
        if k == self.policy.num_gpu_batches - 1:
            ...
            ((w_q, _), (w_k, _), (w_v, _),
             (w_out, _), (w_ln, _)) = weight_read_buf.val
...
class LlamaMLP:
    ...
    def init_weight(self, weight_home, path):
        h, dtype = (self.config.input_dim, self.config.dtype)
        mlp_h = self.config.ffn_embed_dim
        path = os.path.join(os.path.join(path, f"model.layers.{self.layer_id}."))
        weight_specs = [
            # w_gate (up_proj.weight for gate)
            ((mlp_h, h), dtype, path + "mlp.gate_proj.weight"),
            # w_up (up_proj.weight for value) - note: in Llama it's combined
            ((mlp_h, h), dtype, path + "mlp.up_proj.weight"),
            # w_down (down_proj.weight)
            ((h, mlp_h), dtype, path + "mlp.down_proj.weight"),
            # w_ln (post_attention_layernorm.weight)
    ...
    def forward(self, hidden, cache_read_buf, weight_read_buf, attention_mask,
        ...
        if k == self.policy.num_gpu_batches - 1:
            # Clear the weight_read_buf if it is the last gpu batch
            ((w_gate, donate[1]), (w_up, donate[2]), (w_down, donate[3]),
             (w_ln, donate[4])) = weight_read_buf.pop()
        else:
            ((w_gate, _), (w_up, _), (w_down, _),
             (w_ln, _)) = weight_read_buf.val

        # Llama MLP: gate_proj * up_proj passed through SiLU, then down_proj
        h = self.compute.llama_mlp(h, w_gate, w_up, w_down, w_ln, donate, self.config.layer_norm_eps)
        hidden.val = h
...
class LlamaLM:
    ...
    def delete_weight(self, j, k):
        v = self.weight_home[j].pop()
        if v:
            if isinstance(v, ValueHolder):
                v = v.val
    ...
    def generate(self,
                 ...
                 verbose: int = 0):
        # Use same generation logic as OptLM (code reuse)
        from flexllmgen.flex_opt import OptLM as BaseOptLM
        ...
        # Generation with default (non-overlapping) for simplicity
        ...
        # Decode and return
        if not verbose:
            output_ids = self.output_ids[:, prompt_len:self.task.prompt_len+self.execute_gen_len]
        else:
            # return with timer info
            output_ids = self.output_ids[:, prompt_len:self.task.prompt_len+self.execute_gen_len]
    ...
    def generation_loop_debug_single_batch(self):
        """Debug mode with single batch."""
        for i in range(min(2, self.execute_gen_len)):
            for j in range(min(2, self.num_layers)):
                for k in range(self.num_gpu_batches):
                    if j == 0:
                        self.load_hidden(i, j, k)
                    self.update_attention_mask(i, k)
    ...
    def generation_loop_debug_multi_batch(self):
        """Debug mode with multiple batches."""
        for i in range(min(2, self.execute_gen_len)):
            for j in range(min(2, self.num_layers)):
                for k in range(min(2, self.num_gpu_batches)):
                    if j == 0:
                        self.load_hidden(i, j, k)
                    self.update_attention_mask(i, k)
                    self.load_weight(i, j, k)
    ...
    def generation_loop_debug_normal(self):
        """Debug mode with execution time breakdown."""
        for i in range(min(2, self.execute_gen_len)):
            for j in range(min(2, self.num_layers)):
                for k in range(self.num_gpu_batches):
                    if j == 0:
...
def run_flexllmgen(args):
    ...
    # Auto-detect model type and load config
    ...
    if model_config.model_type == "llama":
        model = LlamaLM(model_config, env, args.path, policy)
    else:
        model = OptLM(model_config, env, args.path, policy)
