...
int main(int argc, char ** argv) {
    ...
    if (argc == 1 || argv[1][0] == '-') {
        printf("usage: %s MODEL_PATH [N_KV_MAX] [IS_PP_SHARED] [VRAM_BUDGET_GB] [MMQ] <PP> <TG> <PL>\n" , argv[0]);
        printf("  <PP>, <TG> and PL are comma-separated lists of numbers without spaces\n\n");
        printf("  example: %s ggml-model-f16.gguf 2048 0 16.0 0 128,256,512 128,256 1,2,4,8,16,32\n\n", argv[0]);
        return 1 ;
    }
    ...
    if (argc >= 5) {
        vram_budget_gb = std::atof(argv[4]);
    }
    ...
    model_params.vram_budget_gb = vram_budget_gb;
    ...
    LOG_TEE("\n");
    LOG_TEE("%s: n_kv_max = %d, is_pp_shared = %d, vram_budget_gb = %.1f, mmq = %d\n", __func__, n_kv_max, is_pp_shared, vram_budget_gb, mmq);
    LOG_TEE("\n");
