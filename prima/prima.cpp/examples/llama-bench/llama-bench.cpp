...
int main(int argc, char ** argv) {
    ...
    for (const auto & inst : params_instances) {
        ...
        if (cparams.n_world == 1) {
            cparams.n_layer_window[0] = llama_model_n_layers(lmodel);
        }
        ...
        if (llama_context_setup_backend(lmodel, cparams, ctx) == nullptr) {
            fprintf(stderr, "%s: error: failed to setup context with model '%s'\n", __func__, inst.model.c_str());
            llama_free(ctx);
            llama_free_model(lmodel);
            return 1;
        }
